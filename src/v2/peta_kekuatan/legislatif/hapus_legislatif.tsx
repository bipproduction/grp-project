...
export function ButtonDeleteLegislatif({ setId, setTingkat, setNama }: any) {
  ...
  const [inputSearchNasional, setInputNasional] = useAtom(
    _dataSearchLegislatifNasional
  );
  const [inputSearchProvinsi, setInputSearchProvinsi] = useAtom(
    _dataSearchLegislatifProvinsi
  );
  const [inputSearchKabKot, setInputSearchKebKot] = useAtom(
    _dataSearchLegislatifKabKot
  );
    ...
    fetch(api.apiLegislatifHapus + `?id=${setId}`).then(async (res) => {
      if (res.status === 200) {
        ...
        if (setTingkat == 1) {
          ...
          desk_log =
            "User menghapus data legislatif tingkat DPRD Kabupaten/Kota";
        }
