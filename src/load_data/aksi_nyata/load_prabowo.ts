...
import {
  ...
  ModelListUndanganPrabowoNew,
  ModelRencanaKunjungan,
} from "@/model/model_aksi_nyata";
import { atom } from "jotai";
import { atomWithStorage } from "jotai/utils";

export const _dataRencanaKunjunganPrabowo = atomWithStorage<
  ModelRencanaKunjungan[]
>("_rencana_kunjungan", []);

export const _dataListUndanganPrabowo = atomWithStorage<
  ModelListUndanganPrabowoNew[]
>("_list_undangan", []);
...
export async function _loadDataRencanaKunjunganPrabowo(
  ...
  await fetch(
    api.apiRencanaKunjunganPrabowoSearch + `?search=${search}&page=${page}`
  )
    .then((res) => res.json())
...
export async function _loadDataListUndanganPrabowo(
  ...
  await fetch(
    api.apiListUndanganPrabowoSearch + `?search=${search}&page=${page}`
  )
    .then((res) => res.json())
